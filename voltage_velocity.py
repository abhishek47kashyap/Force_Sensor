...
while not rospy.is_shutdown():
    ...
    if sensor4 >= 2.70 and sensor4 <= 2.75:
    # if sensor1 >= 2.72 and sensor1 <= 2.75:
        print "sensor value = {0:.2f}, home".format(sensor4)
        if msg.position[0] > 0.0:
            msg.position[0] = msg.position[0] - (outer_yaw_joint_vel*0.01)
        elif msg.position[0] < 0.0:
            msg.position[0] = msg.position[0] + (outer_yaw_joint_vel * 0.01)
        # msg.position = [0.0]
